                      ...
                      <div class="box-body">
                        <div class="table-responsive">
                          ...
                          <form action="route/<?php echo $data; ?>/generate_purchase.php" method="post">
                            <button type="submit" class="btn btn-success mb-3"><i class="fas fa-save"></i> Terbit PO</button>
                            <table id="example1" class="table table-bordered table-striped">
                              ...
                              <tbody>
                                <?php
                                $sql1 = mysqli_query($koneksi, "SELECT $tabel.kd_beli,$tabel.ppn,$tabel.status_pembelian,$tabel.tgl_beli,$tabel.kd_supp,
                                 supplier.nama as nama_supplier 
                                from $tabel
                                JOIN supplier on supplier.kd_supp = $tabel.kd_supp  WHERE status_pembelian = 0;");
                                $no = 1;
                                $nilai_pjk = 0;
                                $subtotal = 0;
                                ...
                                while ($s1 = mysqli_fetch_array($sql1)) {
                                  $sql2 = mysqli_query($koneksi, "SELECT sum(disc) as tot_disc, sum(jml*price) as tot_price from $tabel2 WHERE kd_beli='$s1[kd_beli]' ");
                                ...
                                ?>
                                  <tr align="left">
                                    <?php if ($s1[$f8] == 0) { ?>
                                      <td><input type="checkbox" name="selected_items[]" value="<?php echo $s1['kd_beli']  ?>" class="largeCheckbox"></td>
                                    <?php } else { ?>
                                      <td></td>
                                    <?php } ?>
                                    <td><?php echo $no; ?></td>
                                    ...
                                    <td style="text-align:right;"><?php echo format_rupiah((int) $s2['tot_price']); ?></td>
                                    <td style="text-align:right;"><?php echo format_rupiah((int) $s2['tot_disc']); ?></td>
                                    <td style="text-align:right;"><?php echo format_rupiah((int) $nilai_pjk); ?></td>
                                    <td style="text-align:right;"><?php echo format_rupiah((int) $subtotal); ?></td>
                                    <td>
                                      <?php if ($s1[$f8] == 0) { ?>
                                      ...
                                      <?php } else { ?>
                                        <button class="btn btn-primary btn-sm elevation-2" style="opacity: .4;" disabled><i class="fa fa-check"></i> Edit</button>
                                        <button class="btn btn-danger btn-sm elevation-2" style="opacity: .4;width:80px" disabled><i class="fa fa-trash"></i> Hapus</button>
                                      <?php } ?>
                                    </td>
                                  </tr>
                                <?php
