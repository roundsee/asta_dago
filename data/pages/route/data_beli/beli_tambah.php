    ...
    <div class="row">
            ...
            <div class="form-group">
              ...
              <select name="<?php echo $f3; ?>" id="supplier_select" class="form-control" required>
                ...
                while ($x = mysqli_fetch_array($query)) {
                  echo "<option value='$x[kd_supp]'>$x[kd_supp] - $x[nama] - $x[term_of_payment]</option>";
                }
                ?>
              </select>
            </div>
      ...
      <div class="col-lg-5">
        <div class="form-group">
          ...
          <input name="<?php echo $f13; ?>" id="ket" class="form-control" rows="3" cols="5" required="required" readonly>
        </div>
      </div>
    </div>
    ...
    <div class="content-wrapper" style="min-height: 10%; padding-bottom: 1px;">
      ...
      <script>
        ...
        function hitungHargaTotal() {
          const jumlah = cleanCurrency(jumlahInput.value) || 0;
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    var hargaDasar = 0; // Variabel global untuk menyimpan harga awal
    ...
    $('#kd_acc').on('change', function() {
      ...
      if (kodeBarang) {
        $.ajax({
          ...
          success: function(data) {
            // Simpan harga awal ke variabel global (hargaDasar)
            hargaDasar = parseFloat(data.replace(/[^0-9,-]+/g, '')) || 0;

            // Tampilkan harga awal dengan format ribuan
    ...
    function updateTotalPcs() {
      ...
      // Pastikan menggunakan harga awal dari variabel global (hargaDasar)
      console.log("Harga awal (dasar):", hargaDasar);

      // Hitung harga setelah dikalikan dengan total pcs
      var hargaXIsi = hargaDasar * (parseFloat(totalPcs) || 0);
