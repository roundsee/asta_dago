...
$judulform = "Penerimaan Barang";

$data = 'data_penerimaan_barang_internal';
$rute = 'penerimaan_barang_internal';
$aksi = 'aksi_penerimaan_barang_internal';
$view = 'penerimaan_barang_internal_view';
$view2 = 'penerimaan_barang_internal_view_detail_terima';

$rute_detail = 'penerimaan_barang_internal_detail';
...
?>
                                                    ...
                                                    <table id="example1" class="table table-bordered table-striped">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            $sql1 = mysqli_query($koneksi, "
                                                                SELECT $tabel.*, pd.$ff5 AS nama_barang, pd.$ff4 AS kd_barang, pd.$ff6 AS qty_diajukan,
                                                                ...
                                                                WHERE $tabel.kd_cus_peminta = '$kd_cus' and status_permintaan <= 1
                                                            ...
                                                            ?>
                                                                <tr align="left">
                                                                    ...
                                                                    <td><?php echo $s1[$f2] . " - " . $s1['nama_pelanggan_peminta']; ?></td>
                                                                    <td><?php echo $s1[$f3] . " - " . $s1['nama_pelanggan_pengirim']; ?></td>
                                                                    ...
                                                                    <td>
                                                                        <?php if ($s1[$f5] <= 1) { ?>
                                                                            <button class="btn btn-success btn-sm elevation-2 btn-terima-barang"
                                                                                ...
                                                                                data-kd_cus_peminta="<?php echo $s1['kd_cus_peminta']; ?>"
                                                                        ...
                                                                        <a href="route/<?php echo $data; ?>/cetak_penerimaan_barang_internal.php?kode_permintaan=<?php echo $s1['kode_permintaan']; ?>">
            ...
            <div class="modal fade" id="modalSuratJalan" tabindex="-1" role="dialog" aria-labelledby="modalSuratJalanLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <div class="modal-content" style="border-radius: 10px; box-shadow: 0px 4px 20px rgba(0, 0, 0, 0.1);">
                        <form id="formSuratJalan" action="route/<?php echo $data ?>/aksi_penerimaan_barang_internal.php" method="POST">
                            ...
                            <?php
                            $_SESSION['submit_token'] = bin2hex(random_bytes(16)); // token unik acak 
                            ?>
                            <div class="modal-body" style="padding: 1.5rem;">
                                ...
                                <input type="hidden" name="kd_cus_peminta" id="kd_cus_peminta">
                                    ...
                                    <table id="modalTable" class="table table-bordered table-striped">
                                        <thead style="background-color: lightgray;">
                                            <tr>
                                                ...
                                                <th style="width: 200px;">Qty Terima</th>
                                                <th style="width: 600px;">Status Barang</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                            ...
                            <div class="modal-footer" style="border-top: none;">
                                <button type="button" id="qty_value" class="btn btn-primary" style="border-radius: 30px; padding: 0.5rem 1.5rem;">Proses Qty Terima</button>
                                ...
                                <button type="submit" id="submitBtn" class="btn btn-success" style="border-radius: 30px; padding: 0.5rem 1.5rem;">Proses Penerimaan</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <script>
                document.getElementById('formSuratJalan').addEventListener('submit', function() {
            ...
            <script>
                $(document).on('click', '.btn-terima-barang', function() {
                    ...
                    var kd_cus_peminta = $(this).data('kd_cus_peminta')

                    $('#kode_permintaan').val(kode_permintaan); // Set nilai ke input hidden modal
                    $('#kd_cus_peminta').val(kd_cus_peminta); // Set judul modal
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            $.each(response, function(index, item) {
                                ...
                                var kd_cus_pengirim = item.kd_cus_pengirim;
                                ...
                                $('#modalTable tbody').append(
                                    ...
                                    '<input type="hidden" name="kd_cus_pengirim" value="' + kd_cus_pengirim + '">' +
                                    ...
                                    '<input type="text" class="form-control qty-terima" name="qty_terima[]" placeholder="Masukkan Qty Kirim" required value = "0" onfocus="if(this.value === \'0\') this.value = \'\';" onblur="if(this.value === \'\') this.value = \'0\';"></td>' +
                ...
                // Fungsi untuk mengatur readonly status pada qty_terima saat pertama kali
                function setInitialReadonlyStatus() {
                    $('#modalTable tbody tr').each(function() {
                        var row = $(this); // Elemen baris saat ini
                        var qtyTerimaInput = row.find('.qty-terima'); // Input qty_terima
                        var qty_terkirim = parseFloat(row.find('.qty-terkirim').data('qty')) || 0; // qty terkirim
                        var qty_diajukan = parseFloat(row.find('.qty-diajukan').data('qty')) || 0; // qty diajukan
                        var qty_diterima = parseFloat(row.find('.qty-diterima').data('qty')) || 0; // qty diterima
                        var qtyTerima = parseFloat(qtyTerimaInput.val().replace(/\./g, '').replace(',', '.')) || 0; // Input qty_terima

                        var qty_total_terima = qty_diterima + qtyTerima;

                        console.log("Qty Diajukan:", qty_diajukan);
                        console.log("Qty Diterima:", qty_diterima);
                        console.log("Qty Terima Input:", qtyTerima);
                        console.log("Total Qty Terima:", qty_total_terima);

                        // Tentukan status barang
                        var status_barang = (qty_total_terima >= qty_diajukan) ? 'Selesai' : 'Belum Selesai';

                        // Kondisi: Jika qty total terkirim sama atau lebih besar dari qty diajukan
                        if (qty_total_terima >= qty_diajukan) {
                            qtyTerimaInput.prop('readonly', true).val(0);
                            row.find('.status-barang').val(status_barang);
                        }

                        // Kondisi: Jika qty terkirim = 0
                        if (qty_terkirim === 0) {
                            qtyTerimaInput.prop('readonly', true).val(0);
                            row.find('.status-barang').val('Belum Melakukan Pengiriman');
                        }
                    });
                }
                ...
                $(document).on('input', '.qty-terima', function() {
                    ...
                    var qty_diajukan = parseFloat(qtyTerimaInput.closest('tr').find('.qty-diajukan').data('qty')) || 0;
                    var qty_diterima = parseFloat(qtyTerimaInput.closest('tr').find('.qty-diterima').data('qty')) || 0;
                    var qty_terkirim = parseFloat(qtyTerimaInput.closest('tr').find('.qty-terkirim').data('qty')) || 0;
                    var qtyTerima = parseFloat(qtyTerimaInput.val().replace(/\./g, '').replace(',', '.')) || 0;

                    var qty_total_terima = qty_diterima + qtyTerima;

                    var terima_total = qty_diajukan - qty_diterima;

                    console.log("Qty Diajukan:", qty_diajukan);
                    console.log("Qty Diterima:", qty_diterima);
                    console.log("Qty Terima Input:", qtyTerima);
                    console.log("Total Qty Terima:", qty_total_terima);
                    console.log("Total Qty terkirim:", qty_terkirim);
                    console.log("terima total:", terima_total);

                    // Validasi jika qty terima melebihi qty diajukan
                    if (qty_total_terima > qty_diajukan || qtyTerima > qty_terkirim) {
                        alert('Qty terima melebihi Qty Diajukan. Mohon periksa kembali!');
                        qtyTerimaInput.val(terima_total); // Set qty kirim maksimal sesuai qty PO
                        // qtyTerimaInput.val(formatRupiah(terima_total)); // Set qty kirim maksimal sesuai qty PO
                        qty_total_terima = qty_diajukan; // Paksa nilai tetap pada batas maksimum
                    }


                    // Update status barang hanya jika input valid
                    var status_barang = (qty_total_terima == qty_diajukan) ? 'Selesai' : 'Belum Selesai';
                    qtyTerimaInput.closest('tr').find('.status-barang').val(status_barang);
                });
                $(document).ready(function() {
                    $('#qty_value').on('click', function() {
                        $('#modalTable tbody tr').each(function() { // Loop through all rows
                            var row = $(this);
                            var qtyTerimaInput = row.find('.qty-terima');
                            ...
                            qtyTerimaInput.val(formatRupiah(qty_terkirim - qty_diterima));
                            row.find('.status-barang').val('Belum Selesai');

                            if (qty_terkirim >= qtyDiajukan && qty_diterima < qtyDiajukan) {
                                qtyTerimaInput.val(formatRupiah(qty_terkirim - qty_diterima));
                ...
                $(document).on('submit', 'form', function(event) {
                    ...
                    $('.qty-kirim').each(function() {
                        ...
                        qtyTerimaInput.val(qtyTerima); // Set nilai tanpa pemisah ribuan
                    });
                });
            </script>
