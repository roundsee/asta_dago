...
$rute_detail = 'good_receiving_detail';
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                              <thead style="background-color: lightgray;" class="elevation-2">
                                <tr>
                                  ...
                                  <th style="text-align: right;"> <?php echo $j5; // Jumlah 
                                                                        ?></th>
                                  <th style="text-align: right;"> <?php echo $j6; // Jumlah Datang 
                                                                        ?></th>
                                  <th>Aksi</th>
                                </tr>
                              </thead>
                              <tbody>
                                <?php
                                // Query SQL untuk mengambil data dari tabel penerimaan_barang
                                $sql1 = mysqli_query($koneksi, "SELECT $f1, $f2, $f3, $f4, $f5 AS total_jumlah, $f6 AS total_datang FROM $tabel ");
                                $no = 1;
                                while ($s1 = mysqli_fetch_array($sql1)) {
                                ?>
                                  <tr align="left">
                                    <td><?php echo $no; ?></td>
                                    <td><a href="main.php?route=<?php echo $rute_detail; ?>&act&id=<?php echo $s1[$f2]; ?>&asal=<?php echo $rute; ?>" title="Detail"><?php echo $s1[$f2]; ?></a></td>
                                    <td><?php echo $s1[$f1]; // Kode PO 
                                        ?></td>
                                    <td><?php echo $s1[$f3]; 
                                        ?></td>

                                    <td style="text-align:right;"><?php echo format_rupiah($s1['total_jumlah']); // Total Jumlah 
                                                                  ?></td>
                                    <td style="text-align:right;"><?php echo format_rupiah($s1['total_datang']); // Total Jumlah Datang 
                                                                  ?></td>
                                    <td>
                                      <a href="route/<?php echo $data; ?>/cetak_terima_barang.php?surat_jalan=<?php echo $s1['surat_jalan']; ?>" target="_blank">
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="viewModalLabel">Good Receiving Detail</h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <!-- Detail penerimaan barang akan dimuat di sini melalui Ajax -->
            </div>
          </div>
        </div>
      </div>
      ...
      <script>
        $(document).ready(function() {
          ...
          $('#viewModal').on('show.bs.modal', function(event) {
            ...
            var surat_jalan = button.data('surat_jalan'); // Ambil data surat_jalan dari tombol

            // Muat data penerimaan barang dengan Ajax
            $.ajax({
              url: 'route/data_penerimaan_barang/detail_penerimaan_barang.php', // Ubah dengan path yang sesuai
              type: 'GET',
              data: {
                surat_jalan: surat_jalan
              }, // Kirim parameter surat_jalan
              success: function(response) {
                $('#viewModal .modal-body').html(response); // Tampilkan respon di modal
              },
              error: function() {
